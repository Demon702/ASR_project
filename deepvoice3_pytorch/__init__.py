...
from deepvoice3_pytorch.speakerencoder import SpeakerEncoder
...
class MultiSpeakerTTSModel(nn.Module):
    ...
    def __init__(self, seq2seq, postnet,
        ...
        if n_speakers > 1:
            ...
            self.embed_speakers = SpeakerEncoder(device , device)
            self.embed_speakers.load_state_dict(torch.load("data/model2/2.pt")["model_state"])
    ...
    def forward(self, text_sequences, mel_targets=None, speaker_ids=None,
        ...
        print("------------------------------------Mel Targets Shape---------------------------")
        print(mel_targets.shape)
        if speaker_ids is not None:
            assert self.n_speakers > 1
            speaker_embed = self.embed_speakers(mel_targets)
        else:
            speaker_embed = None
